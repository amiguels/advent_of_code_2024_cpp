...
bool check(auto& levels){
    //  x < 0 ? true : false
    bool direction{std::signbit(static_cast<int32_t>(levels.at(0) - static_cast<int32_t>(levels.at(1))))}; 
    
    for (auto it = levels.begin(); it < levels.end() - 1; it++ )
    {
        auto& this_level = *it;
        auto& next_level = *(it+1);

        int32_t delta = static_cast<int32_t>(this_level) - static_cast<int32_t>(next_level);

        bool next_direction{std::signbit(delta)};
        ...
        int32_t abs_delta = std::abs(delta);
...
bool check_reduced(auto& levels){
    for (uint32_t index_to_avoid = 0; index_to_avoid < levels.size() ; index_to_avoid++)
    {
        std::vector<uint32_t> reduced_vector{levels};
        reduced_vector.erase(reduced_vector.begin() + index_to_avoid);

        bool are_reduced_levels_ok{check(reduced_vector)};

        if (are_reduced_levels_ok)
        {
            return true;
        }
    }
